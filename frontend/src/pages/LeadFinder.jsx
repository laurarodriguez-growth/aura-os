...
export default function LeadFinder() {
  ...
  return (
    <>
      <PageHeader title="Generar leads" description="Busca negocios con tu API, define cómo calificarlos y guarda cada resultado en la base permanente." />
          ...
          <section className="scoring-box">
            ...
            {scoringOpen && (
              <div className="scoring-editor">
                <div className="scoring-mode-grid">
                  <button type="button" className={scoringMode === 'automatic' ? 'active' : ''} onClick={() => chooseMode('automatic')}>
                    <Sparkles size={17} /><strong>Automático</strong><small>Preestablecido para {form.niche} en Panamá y editable.</small>
                  </button>
                ...
                {scoringMode === 'template' && (
                  <div className="template-picker">
                    ...
                    {selectedTemplateId && <button type="button" className="button ghost danger-button" onClick={deleteTemplate}><Trash2 size={15} />Eliminar</button>}
                  </div>
                )}
                ...
                {scoringBusy ? <div className="scoring-loading">Cargando configuración…</div> : (
                  <>
                    <div className="rules-header">
                      ...
                      <button type="button" className="button secondary compact" onClick={addRule}><Plus size={15} />Agregar criterio</button>
                    </div>
                        ...
                        {rules.map((rule, index) => {
                          ...
                          return (
                            <div className={`scoring-rule ${rule.enabled ? '' : 'disabled'}`} key={`${rule.criterion}-${index}`}>
                              ...
                              <label>Criterio
                                <select value={rule.criterion} onChange={(e) => changeCriterion(index, e.target.value)}>
                              ...
                              <label>Puntos
                                <input type="number" min="-100" max="100" value={rule.points} onChange={(e) => updateRule(index, { points: Number(e.target.value) })} />
                              </label>
                              <button type="button" className="icon-button rule-delete" onClick={() => removeRule(index)} title="Eliminar criterio"><Trash2 size={16} /></button>
                            </div>
                          );
                        })}
                      </div>
                    ...
                    <div className="tier-editor">
                      ...
                      <label>Tier A desde<input type="number" min="0" max="100" value={thresholds.A} onChange={(e) => setThresholds({ ...thresholds, A: Number(e.target.value) })} /></label>
                      <label>Tier B desde<input type="number" min="0" max="100" value={thresholds.B} onChange={(e) => setThresholds({ ...thresholds, B: Number(e.target.value) })} /></label>
                      <label>Tier C desde<input type="number" min="0" max="100" value={thresholds.C} onChange={(e) => setThresholds({ ...thresholds, C: Number(e.target.value) })} /></label>
                    </div>

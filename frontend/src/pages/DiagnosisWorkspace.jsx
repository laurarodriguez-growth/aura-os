...
export default function DiagnosisWorkspace() {
  ...
  const report = definitive ? (
    <section className="diagnosis-section-stack">
      <section className="panel report-ready-card"><FileText size={34} /><p className="eyebrow">INFORME DIAGNÓSTICO AURA</p><h2>De las señales a una fuga confirmada</h2><p>Incluye el recorrido Consulta → Respuesta → Seguimiento → Cita → Venta, evidencia, hallazgos, riesgo, impacto comercial, prioridades, roadmap y límites de validación. La prescripción comercial se mantiene como una fase separada.</p><div className="report-type-actions"><button className="button secondary" onClick={() => createReport('preliminary')} disabled={saving}>Informe preliminar</button><button className="button diagnose-primary" onClick={() => createReport('final')} disabled={saving || !definitive.report_readiness.final_ready}>Informe final</button></div></section>
      ...
      <section className="panel diagnosis-phase-boundary"><div className="section-heading"><div><p className="eyebrow">SIGUIENTE FASE</p><h3>Prescripción comercial separada del diagnóstico</h3><p>El Diagnóstico AURA confirma qué está ocurriendo y qué debe corregirse primero. La selección de plan, plataforma, alcance y precio se realizará en el módulo de Prescripciones cuando esa fase sea aprobada.</p></div></div></section>
    </section>
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<><Link className="button secondary" to="/diagnose/list"><ArrowLeft size={16} />Diagnósticos AURA</Link><button className="button secondary" onClick={load}><RefreshCw size={16} />Actualizar</button></>}
      />

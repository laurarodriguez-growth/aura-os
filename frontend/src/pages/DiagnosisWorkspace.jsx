...
import { ArrowLeft, BrainCircuit, CheckCircle2, FileSearch, FileText, FolderOpen, MessageSquareText, RefreshCw, Route, Save } from 'lucide-react';
...
export default function DiagnosisWorkspace() {
  ...
  const load = async () => {
    ...
    try {
      const [data, definitiveData, profileRows] = await Promise.all([
      ...
      setDiagnosis(data); setDefinitive(definitiveData); setProfiles(profileRows || []);
      setImplementation({
  ...
  if (!diagnosis || !form || !definitive || !implementation) return <section className="panel"><h2>Diagnóstico no disponible</h2><p>{error}</p></section>;

  const summary = (
    <section className="diagnosis-section-stack">
      <DiagnosisDefinitiveOverview data={definitive} />
  ...
  let content = summary;
  if (section === 'interview') content = <DiagnosisConversation diagnosisId={diagnosisId} data={definitive} onChanged={load} />;
  if (section === 'evidence') content = <DiagnosisEvidence diagnosisId={diagnosisId} items={definitive.evidence} analysis={diagnosis.latest_analysis} requirements={definitive.evidence_requirements} blocks={definitive.blocks} restricted={definitive.evidence_restricted} onChanged={load} />;
  ...
  return <><PageHeader title={diagnosis.company_name} description={`${diagnosis.industry || 'Empresa'} · Madurez ${definitive.metrics.maturity ?? 'sin evaluar'} · Cobertura ${definitive.metrics.evidence_coverage}%`} actions={<><Link className="button secondary" to="/diagnose/list"><ArrowLeft size={16} />Diagnósticos</Link><button className="button secondary" onClick={load}><RefreshCw size={16} />Actualizar</button></>} />{error && <div className="form-error page-error">{error}</div>}{message && <div className="diagnose-success page-message"><CheckCircle2 size={17} />{message}</div>}<div className="diagnosis-workspace"><aside className="diagnosis-workspace-nav"><p className="eyebrow">WORKSPACE</p>{sections.map(({ id, label, icon: Icon }) => <NavLink key={id} to={`/diagnose/${diagnosisId}/${id}`} className={({ isActive }) => isActive ? 'active' : ''}><Icon size={17} /><span>{label}</span>{id === 'interview' && <small>{definitive.questions.filter((item) => item.status === 'pending').length}</small>}{id === 'evidence' && <small>{definitive.metrics.evidence_coverage}%</small>}</NavLink>)}</aside><main className="diagnosis-workspace-content">{content}</main></div></>;
}

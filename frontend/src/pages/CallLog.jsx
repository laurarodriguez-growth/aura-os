...
export default function CallLog() {
  ...
  return (
    <>
      <PageHeader title="Call Log" description="Una fila por intento de contacto para medir actividad, conexión, reuniones y ventas." actions={<><button className="button secondary" onClick={load}><RefreshCw size={16} />Actualizar</button><button className="button primary" onClick={() => downloadExport('/api/export/call-logs', `aura-grow_call_log_${new Date().toISOString().slice(0, 10)}.csv`).catch((e) => setError(e.message))}><Download size={16} />Exportar CSV</button></>} />

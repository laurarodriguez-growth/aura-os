...
export default function DiagnosisReport() {
  ...
  return <main className="diagnosis-report-page"><div className="report-toolbar no-print"><Link className="button secondary" to={`/diagnose/${diagnosisId}/report`}><ArrowLeft size={16} />Volver a Diagnose</Link><button className="button diagnose-primary" onClick={() => window.print()}><Printer size={17} />Guardar como PDF</button></div><article className="diagnosis-report-sheet">
    <header className="report-cover"><div className="report-brand"><span className="report-brand-logo"><AuraLogo /></span><div><strong>AURA GROW</strong><small>Diagnose · by Laura Rodriguez</small></div></div><p>{reportType === 'final' ? 'INFORME FINAL VALIDADO' : 'INFORME PRELIMINAR'}</p><h1>{diagnosis.company_name}</h1><h2>{diagnosis.industry || 'Diagnóstico AURA · Sistema de Conversión Comercial'}</h2><div className="report-cover-score"><strong>{definitive.metrics.maturity ?? '—'}</strong><span>/100</span><small>Índice interno · evidencia {definitive.metrics.evidence_coverage}%</small></div><footer><span>{diagnosis.city || 'Panamá'}</span><span>{new Date().toLocaleDateString('es-PA', { year: 'numeric', month: 'long', day: 'numeric' })}</span></footer></header>
    ...
    <section className="report-section report-page-break"><p className="report-kicker">02 · RECORRIDO DE CONVERSIÓN</p><h2>Consulta → Respuesta → Seguimiento → Cita → Venta</h2><div className="report-process-xray">{definitive.process_xray.map((stage) => <article key={stage.label}><span className={`stage-dot ${stage.visual_status}`} /><div><strong>{stage.label}</strong><small>{statusLabels[stage.visual_status]}</small>{stage.finding && <p>{stage.finding}</p>}</div></article>)}</div></section>
    ...
    <section className="report-section report-closing"><p className="report-kicker">CIERRE</p><h2>Diagnose piensa. Focus ejecuta.</h2><p>Este informe confirma qué ocurre, qué evidencia lo demuestra y qué debe corregirse primero. La prescripción comercial es la siguiente fase y se mantiene separada para no convertir una conclusión diagnóstica en una venta automática.</p><div className="report-signature"><strong>Laura Rodriguez</strong><span>Growth Strategist · Sistemas de Conversión Comercial</span></div></section>
  </article></main>;
}

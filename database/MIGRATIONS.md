...
## 25–26 · Pre-Diagnóstico AURA

- `25_prediagnosis_aura.sql`: crea el registro histórico de Pre-Diagnósticos AURA vinculado a `leads`, sin modificar ni borrar datos existentes.
- `26_verify_prediagnosis_aura.sql`: verificación de solo lectura de tabla, índices y RLS.

Orden seguro de despliegue: respaldo → migración 25 → verificación 26 → backend → frontend Aura OS → prueba end-to-end → landing pública.
